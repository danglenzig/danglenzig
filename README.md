# Brad Neal

**Technical Generalist | Game Developer | Backend / AI-Adjacent Engineer**  
Simrishamn, Skåne, Sweden  
Dual citizen USA / Sweden  
Phone: 46 70-234-4921  
Email: bradneal.gp@gmail.com  
AAA Game Contributions: [https://www.mobygames.com/person/672830/brad-neal/credits/]
GitHub: [https://github.com/danglenzig]  
LinkedIn: [https://www.linkedin.com/in/brad-neal-421473138/]
Itch.io: [https://americanwerewolf.itch.io/]

## Profile

Technically versatile developer with 25+ years of experience across network engineering, game development, and systems work. Strong background in debugging complex systems, building reliable infrastructure, and communicating clearly under pressure. Currently studying game programming at Forsbergs while independently upskilling in Python, AI engineering, agents, RAG, and MCP to broaden employability across games, tools, backend, and AI-adjacent roles.

## Core Strengths

- Systems thinking and problem solving.
- Backend, networking, and infrastructure.
- Game programming and technical design.
- Tooling, workflow automation, and technical documentation.
- Python and applied AI engineering.
- Reliable delivery in high-pressure environments.
- Cross-functional communication with technical and non-technical teams.

## Technical Skills

**Languages:** C, C++, Lua, Python, GDScript, .NET  
**Game / Creative Tech:** Godot, Unity, Unreal, Roblox, Raylib  
**AI / Backend / Dev Tools:** Git, GitHub, Docker, FastAPI, Pydantic, LangChain, LangGraph, MCP, Ollama, Hugging Face, Redis, Neo4j  
**Concepts:** Algorithms, data structures, OOP, network programming, load balancing, distributed systems, prompt engineering, RAG, memory systems, multi-modal AI

## Education

**Forsbergs Skola, Stockholm**  
Game Programming Vocational Diploma  
2025 – Present

Relevant coursework: C, C++, algorithms, data structures, AI programming, backend/network programming, Unity, Unreal

**Futuregames, Stockholm**  
Game Design Vocational Diploma,
2022 – 2024

**United States Air Force, Keesler AFB**  
Digital Communication Systems

## Independent Study

**Full-Stack Agentic and Generative AI Development With Python*

Hands-on study in Python, Git, Docker, Pydantic, LLMs, prompt engineering, agents, RAG, LangChain, LangGraph, multi-modal AI, FastAPI, and MCP. Focused on building practical AI systems and applying these skills to future software, tools, and game-related projects.

## Experience

**Valley Lodge Games, Simrishamn**  
**Co-Founder / Engineering Lead**  
April 2025 – Present

- Led design and development of *Madame Massacre’s Midnight Matinee*, an episodic point-and-click horror series, in partnership with Game Habitats’ HAVEN incubation program.
- Developed a custom Godot-based framework for 2D narrative adventure gameplay.
- Combined real-world miniature set design with digital character animation and VFX to support a hybrid visual pipeline.
- Supported studio operations, funding strategy, external communication, and release planning.

**The Gang Sweden, Stockholm**  
**Technical Game Designer**  
January 2024 – January 2025

- Contributed to design and implementation tasks within project teams developing games for clients.
- Built tools and supported workflows for designers and production teams.
- Contributed to Roblox projects for external clients including Vans, Dreamworks, PGA Tour, and Royal and Ancient Golf Club.

**Electronic Arts, Austin / Stockholm**  
**Senior Network Engineer**  
March 2013 – June 2022

- Designed and delivered an intercontinental fiber optic network connecting EA data centers.
- Developed application load-balancing solutions and supported network security operations.
- Contributed to disaster preparedness, DDoS mitigation, and launch readiness for major EA titles and studios.

**Various Organizations, USA**  
**Network Engineer**  
1995 – 2013

- Worked in network engineering and architecture roles across enterprise, telecom, finance, government, and military environments.
- Supported infrastructure reliability, communications systems, and operational continuity.

## Projects

**Graduation Project Direction**  
**Agentic Visual Novel System**

Planned project combining game development with practical AI integration. Intended focus areas include interactive narrative systems, agent behavior, and production-minded AI workflows.

## Work Preferences

- Open to full-time, contract, and freelance work.
- Open to game industry, software, backend, tools, and AI-adjacent roles.
- Onsite, hybrid, or remote in Southern Sweden, remote work elsewhere.

## Additional Information

- Strong fit for roles involving systems, tools, workflows, reliability, or applied AI.
- Comfortable working across creative, technical, and operational contexts.